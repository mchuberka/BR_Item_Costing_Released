 ...
 * Date:		2022-08-16
 * File:		BR_WAS_Product_Dev_Calculations.js
 * Version:     0.5
 *
...
//*******************************************************
            ...
            var countryOfOrigin = rec.getValue({fieldId: 'custrecord_br_itdv_country_of_origin'});
            var itemLength = rec.getValue({fieldId: 'custrecord_br_itdv_item_length'});
            var itemWidth = rec.getValue({fieldId: 'custrecord_br_itdv_item_width'});
            var itemHeight = rec.getValue({fieldId: 'custrecord_br_itdv_item_height'});
            var itemWeight = rec.getValue({fieldId: 'custrecord_br_itdv_item_weight'});
            var containerCapacity40HQ = rec.getValue({fieldId: 'custrecord_br_itdv_container_qty_40hq'});
            var useContainerCapacityQty = rec.getValue({fieldId: 'custrecord_br_itdv_use_container_qty'});
            var maxQtyPerLabel = 1;
            ...
            //Calculate Required Values
                ...
                var itemLengthCM = (itemLength * 2.54);
                var itemWidthCM = (itemWidth * 2.54);
                var itemHeightCM = (itemHeight * 2.54);
                //Calculate the CBM
                var unitCBM = ((itemLengthCM * 0.01) * (itemWidthCM * 0.01) * (itemHeightCM * 0.01));
                rec.setValue({fieldId: 'custrecord_br_itdv_item_cbm', value: unitCBM});

                //Amazon Size Tier
                var amazonSizeTier = 2;
                if (itemLength > 18 || itemWidth > 14 || itemHeight > 8 || itemWeightCeil > 20){
                    amazonSizeTier = 1;
                }
                rec.setValue({fieldId: 'custrecord_br_itdv_amazon_size_tier', value: amazonSizeTier});

                //Item Length/Girth
                var itemLengthGirth = (Math.round(itemLength) + (2 * Math.round(itemWidth)) + (2 * Math.round(itemHeight)));
                rec.setValue({fieldId: 'custrecord_br_itdv_item_length_girth', value: itemLengthGirth});

                //Note: FBM Dim Weight is calculated after defaults are imported below.
...
//*******************************************************
        ...
        var defaultsItemInformation = defaultsInformationMapping;
            ...
            var containerEfficiencyOcean = defaultsItemInformation.containerEfficiencyOcean;
            ...
            //Calculate FBM Dim Weight
            var itemDimWeight = ((Math.ceil(Math.round(itemLength)) * (Math.round(itemWidth)) * (Math.round(itemHeight))) / upsDimWeightModifier);
            rec.setValue({fieldId: 'custrecord_br_itdv_item_dimweight', value: itemDimWeight});

            //Calculate ESTIMATED Qty for 40HQ container based on Product CBM
            var estimated40HQcbmQty = (containerLoadableCbmOcean / unitCBM);
            rec.setValue({fieldId: 'custrecord_estimated_qtycontainer_by_cbm', value: Math.floor(estimated40HQcbmQty)});
...
//*******************************************************
    //Tariff
            var tariffInformationMapping = itemCostingLibrary.tariffInformationSearch(htsCode, countryOfOrigin);
                ...
                rec.setValue({fieldId: 'custrecord_br_itdv_tariff_percentage', value: tariffPct.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_tariff_per_unit', value: tariffPerUnit.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_total_tariff', value: tariffTotal.toFixed(2)});
    ...
    //Ocean Lanes
            ...
            var oceanLaneInformationMapping = itemCostingLibrary.oceanLaneInfoSearch(countryOfOrigin);
                ...
                //Set Weighted Lane Cost for point-in-time capture if record is locked.
                rec.setValue({fieldId: 'custrecord_br_itdv_weighted_lane_cost', value: oceanLaneWeightedCost.toFixed(2)});
                ...
                logModule.debug({title: 'Ocean Lane TOTAL', details: oceanLaneWeightedCost});
                rec.setValue({fieldId: 'custrecord_br_itdv_total_sea_freight', value: oceanLaneWeightedCost.toFixed(2)});
    ...
    //Sourcing
            var sourcingInformationMapping = itemCostingLibrary.sourcingInfoSearch(countryOfOrigin);
                ...
                rec.setValue({fieldId: 'custrecord_br_itdv_sourcing_pct', value: sourcingCommissionPct.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_total_sourcing', value: sourcingTotal.toFixed(2)});

    //TOTAL LANDED COST FEES

            var landedCostTotal = (tariffTotal + oceanLaneWeightedCost + sourcingTotal);  //sub oceanLaneWeightedCost for oceanLaneWeightedCost
...
//*******************************************************
    //FBM Transfer Lanes
            ...
            rec.setValue({fieldId: 'custrecord_br_itdv_wgt_transfr_cost_fbm', value: transferLaneWeightedCostFBM.toFixed(2)});
...
//*******************************************************
    //FBA Transfer Lanes
            ...
            rec.setValue({fieldId: 'custrecord_br_itdv_wgt_transfr_cost_fba', value: transferLaneWeightedCostFBA.toFixed(2)});
...
//*******************************************************
    ...
    // logModule.debug({title: 'FBA Storage Cost', details: fbaStorageCost});
            ...
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_cubic_feet', value: fbaCubicFeet.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_storage_cost', value: fbaStorageCost.toFixed(2)});

//*******************************************************
    //FBM Storage -defaulting to zero for now (concept of unlimited Space.
        var fbmStorageCost = 0;
            rec.setValue({fieldId: 'custrecord_br_itdv_fbm_storage_cost', value: fbmStorageCost});
    ...
    //FBA Fulfillment
        ...
        // logModule.debug({title: 'Peak Surcharge Active', details: fbaPeakSurchActive});
                ...
                if (fbaPeakSurchActive === true){
                    ...
                    rec.setValue({fieldId: 'custrecord_br_itdv_fba_peak_surch_active', value: true});
                } else {
                    fbaFulfillFee = fbaFulfillBaseFee;
                    rec.setValue({fieldId: 'custrecord_br_itdv_fba_peak_surch_active', value: false});
                }
        // logModule.debug({title: 'FBA Final Fee', details: fbaFulfillFee});
            ...
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_dim_wt', value: amazonDimWeight.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_fulfill_rec', value: fbaFulfillFeeId});
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_base_fee', value: fbaFulfillFee.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_addl_per_lb', value: amzPerPoundFees.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_fuelsrchg_pct', value: amazonFuelSurchargePct.toFixed(2)});
            ...
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_fuelsrchg_amt', value: amazonFuelSurchargeAmt.toFixed(2)});
            var fbaFulfillTotal = (fbaFulfillSubtotal+ amazonFuelSurchargeAmt);

            //Set Total FBA Fulfillment Fees value
            rec.setValue({fieldId: 'custrecord_br_itdv_fba_total_fulfill', value: fbaFulfillTotal.toFixed(2)});
...
//*******************************************************
    //FBM Picking
        ...
        rec.setValue({fieldId: 'custrecord_br_itdv_fbm_picking_cost', value: itemPickCostFbm.toFixed(2)});
    ...
    //FBM Shipping (Delivered Cost Record)
            ...
            //Large Package Fee Check
                ...
                if (fbmItemLength >= upsLargeLengthRes){
                    ...
                    rec.setValue({fieldId: 'custrecord_br_itdv_res_length_fbm', value: true});
                }
                if (fbmItemLength >= upsLargeLengthCom){
                    shipCostLgPkgLengthCom = shippingLgPkgLengthCom;
                    rec.setValue({fieldId: 'custrecord_br_itdv_com_length_fbm', value: true});
                }
                ...
                if (fbmItemLengthGirth >= upsLargeLenGirRes) {
                    ...
                    rec.setValue({fieldId: 'custrecord_br_itdv_res_len_girth_fbm', value: true});
                }
                if (fbmItemLengthGirth >= upsLargeLenGirCom) {
                    shipCostLgPkgLenGirthCom = shippingLgPkgLenGirthCom;
                    rec.setValue({fieldId: 'custrecord_br_itdv_com_len_girth_fbm', value: true});
                }
        ...
        /*                if (maxQtyPerLabel > upsAddlHandlingPackaging) {   //note - this is the wrong formula if we ever need this calc.
                            ...
                            rec.setValue({fieldId: 'custrecord_br_itdv_ah_pkgng_fbm', value: true});
                        }*/
                ...
                if (fbmItemWidth > upsAddlHandlingWidth){
                    ...
                    rec.setValue({fieldId: 'custrecord_br_itdv_ah_width_fbm', value: true});
                }
                ...
                if (fbmItemLength > upsAddlHandlingLength){
                    ...
                    rec.setValue({fieldId: 'custrecord_br_itdv_ah_length_fbm', value: true});
                }
                ...
                if (fbmItemLengthGirth > upsAddlHandlingLenGir){
                    ...
                    rec.setValue({fieldId: 'custrecord_br_itdv_ah_len_girth_fbm', value: true});
                }
                ...
                if (itemWeightCeil > upsAddlHandlingWeight){
                    ...
                    rec.setValue({fieldId: 'custrecord_br_itdv_ah_weight_fbm', value: true});
                }
            ...
            //Over Maximum Limits Fee Check
                ...
                if (itemWeightCeil > upsOverMaxWeight){
                    ...
                    record.setValue({fieldId: 'custrecord_br_itdv_max_weight_fbm', value: true})
                }
                if (fbmItemLength > upsOverMaxLength){
                    shipCostMaxLimitFee = upsOverMaxLimitFee;
                    record.setValue({fieldId: 'custrecord_br_itdv_max_length_fbm', value: true})
                }
                if (fbmItemLengthGirth > upsOverMaxLenGir){
                    shipCostMaxLimitFee = upsOverMaxLimitFee;
                    record.setValue({fieldId: 'custrecord_br_itdv_max_len_girth_fbm', value: true})
                }
            ...
            //Set FMB Shipping Values
                ...
                //Applies to all
                rec.setValue({fieldId: 'custrecord_br_itdv_base_fee_fbm', value: shippingBaseFee.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_residential_fee_fbm', value: shippingResidentialFee.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_fuel_surch_pct_fbm', value: upsfuelSurchargeDefaultPct.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_total_peak_surch_fbm', value: shippingPeakSurchargeTotal.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_area_surcharge_fbm', value: shippingExtDelvAreaSurch.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_rem_area_surchge_fbm', value: shippingRemoteDelvAreaSurch.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_other_surcharges_fbm', value: upsOtherSurcharge.toFixed(2)});
                //Large Package
                rec.setValue({fieldId: 'custrecord_br_itdv_peak_lgpkg_fbm', value: shipCostPeakSurchLarge.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_lg_package_fee_fbm', value: shipCostLargePackage.toFixed(2)});
                //Additional Handling
                rec.setValue({fieldId: 'custrecord_br_itdv_peak_addlhnd_fbm', value: shipCostPeakSurchAH.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_addl_handling_fbm', value: shipCostAdditionalHandling.toFixed(2)});
                //Over Maximum Limits
                rec.setValue({fieldId: 'custrecord_br_itdv_overmaxlimit_fbm', value: shipCostMaxLimitFee.toFixed(2)});
        ...
        logModule.audit({title: 'Set Calculated Values'});
                rec.setValue({fieldId: 'custrecord_br_itdv_total_delvd_fees_fbm', value: fbmShippingFeesTotal.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_fbm_fuelsrchg_amt', value: upsFuelSurchargeAmount.toFixed(2)});
                rec.setValue({fieldId: 'custrecord_br_itdv_total_delvd_costs_fbm', value: totalFBMShippingCost.toFixed(2)});

                // set total FBM fulfillment fees
                rec.setValue({fieldId: 'custrecord_br_itdv_fbm_total_fulfill', value: totalFbmFulfillmentCost.toFixed(2)});
...
//*******************************************************
    //Sales Commissions
        //FBM
            ...
            //set Field Values
            rec.setValue({fieldId: 'custrecord_br_itdv_salecomm_pct_amt_fbm', value: fbmCommissionPctCost.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_salecomm_per_unit_fbm', value: fbmCommissionPerUnitCost.toFixed(2)});
        ...
        //FBA
            ...
            //set Field Values
            rec.setValue({fieldId: 'custrecord_br_itdv_salecomm_pct_amt_fba', value: fbaCommissionPctCost.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_salecomm_per_unit_fba', value: fbaCommissionPerUnitCost.toFixed(2)});
        ...
        //Bulk
            ...
            //set Field Values
            rec.setValue({fieldId: 'custrecord_br_itdv_salecomm_pct_amt_blk', value: bulkCommissionPctCost.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_salecomm_per_unit_blk', value: bulkCommissionPerUnitCost.toFixed(2)});
...
//*******************************************************
    ...
    var netLandedCost = purchasePrice + landedCostTotal;
            logModule.debug({title: 'Net Landed Cost', details: netLandedCost});
    ...
    // Calculate Amazon Admin Fee
        ...
        var refundAmazonAdminFee = (((amazonRefundAdminPct * 0.01) * fbmCommissionPctCost) + fbmCommissionPerUnitCost);
            ...
            refundAmazonAdminFee = (refundAmazonAdminFee * (refundPct * 0.01));
                logModule.debug({title: 'Refund - Amazon Admin Fee', details: refundAmazonAdminFee});

    // Calculate Inventory Loss
    var refundInventoryLoss = ((refundPct * 0.01) * (1 - (returnShippingPct * 0.01)) * netLandedCost);
                logModule.debug({title: 'Refund - Inventory Loss', details: refundInventoryLoss});
    ...
    var refundUnrecoverableInventory = (((refundPct * 0.01) * (returnShippingPct * 0.01) * (1 - (restockPct * 0.01))) * netLandedCost);
                logModule.debug({title: 'Refund - Unrecoverable Inv', details: refundUnrecoverableInventory});
    ...
    var refundFulfillLossFBA = (((refundPct * 0.01) * fbaFulfillTotal) + transferLaneWeightedCostFBA + fbaStorageCost);
                logModule.debug({title: 'Refund - FulfillLoss FBA', details: refundFulfillLossFBA});

    // Calculate Label Loss FBM
    var refundLabelLossFBM = ((refundPct * 0.01) * totalFBMShippingCost);
                logModule.debug({title: 'Refund - LabelLoss FBM', details: refundLabelLossFBM});

    // Calculate Return Shipping Cost
    var refundReturnShipCost = (((refundPct* 0.01) * (returnShippingPct * 0.01)) * totalFBMShippingCost);
                logModule.debug({title: 'Refund - Return Ship Cost', details: refundReturnShipCost});
    ...
    var refundInventoryCostRecovery = (((refundPct * 0.01) * (returnShippingPct * 0.01) * (restockPct * 0.01)) * netLandedCost);
                logModule.debug({title: 'Refund - Inventory Cost Recovery', details: refundInventoryCostRecovery});
    ...
    var refundTotalFBM = (refundAmazonAdminFee + refundInventoryLoss + refundUnrecoverableInventory + refundPickLossFBM + refundLabelLossFBM + refundReturnShipCost - refundInventoryCostRecovery);
                logModule.debug({title: 'FBM Total Refund', details: refundTotalFBM});
    ...
    var refundTotalFBA = (refundAmazonAdminFee + refundInventoryLoss + refundUnrecoverableInventory + refundFulfillLossFBA + refundReturnShipCost - refundInventoryCostRecovery);
                logModule.debug({title: 'FBA Total Refund', details: refundTotalFBA});

    // Calculate Bulk Total Refund Rate
    var refundTotalBulk = (((bulkRefundPct * 0.01) * (1 + (bulkRefundCostMarkupPct * 0.01))) * basePrice);
                logModule.debug({title: 'Bulk Total Refund', details: refundTotalBulk});

    // Set Field Values
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_admin_fee', value: refundAmazonAdminFee.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_inventory_loss', value: refundInventoryLoss.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_unrecover_inv', value: refundUnrecoverableInventory.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_pick_loss_fbm', value: refundPickLossFBM.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_flfil_loss_fba', value: refundFulfillLossFBA.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_label_loss_fbm', value: refundLabelLossFBM.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_return_ship', value: refundReturnShipCost.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_inv_cost_recov', value: refundInventoryCostRecovery.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_total_fbm', value: refundTotalFBM.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_total_fba', value: refundTotalFBA.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_refund_total_bulk', value: refundTotalBulk.toFixed(2)});
    ...
    //Set Total Sold Cost Values
        ...
        rec.setValue({fieldId: 'custrecord_br_itdv_total_sold_costs_fbm', value: fbmSoldCostTotal.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_total_sold_costs_fba', value: fbaSoldCostTotal.toFixed(2)});
        rec.setValue({fieldId: 'custrecord_br_itdv_total_sold_costs_blk', value: bulkSoldCostTotal.toFixed(2)});
...
//*******************************************************
    //Calculate cost for item and set field value on record
        ...
        rec.setValue({fieldId: 'custrecord_br_itdv_fixed_overhead_amount', value: fixedOverheadAmount.toFixed(2)});
...
//*******************************************************
    ...
    //BULK
        ...
        var customerPaysTariff = rec.getValue({fieldId: 'custrecord_br_itdv_customer_pays_tariff'});
        var customerPaysSeaFreight = rec.getValue({fieldId: 'custrecord_br_itdv_custm_pays_seafreight'});
    ...
    //Set Calculated Margin Values on Form
        ...
        if (fbaItem === true) {          //FBA Item
                //set Primary to 'FBA'
            rec.setValue({fieldId: 'custrecord_br_itdv_primary_class', value: 'FBA'});
                //set Primary Fields
            rec.setValue({fieldId: 'custrecord_br_itdv_contrib_margin_costs', value: fbaContributionMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_contribution_margin', value: fbaContributionMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_contrib_margin_pct', value: fbaContributionMarginPct.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_total_unit_costs', value: fbaGrossMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_gross_margin', value: fbaGrossMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_gross_margin_pct', value: fbaGrossMarginPct.toFixed(2)});

                //Set Alternate to 'FBM'
            rec.setValue({fieldId: 'custrecord_br_itdv_alternate', value: 'FBM'});
            //set Alternate Fields
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_margin_costs', value: fbmContributionMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_margin', value: fbmContributionMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_margin_pct', value: fbmContributionMarginPct.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_unit_costs', value: fbmGrossMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_gross_margin', value: fbmGrossMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_gross_margin_pct', value: fbmGrossMarginPct.toFixed(2)});

            //Check Margin Review Box if Alternate (FBM) Margin > Primary (FBA) Margin


            if (fbmContributionMarginAmt > fbaContributionMarginAmt) {
                rec.setValue({fieldId: 'custrecord_br_itdv_margin_review', value: true});
           } else {
                rec.setValue({fieldId: 'custrecord_br_itdv_margin_review', value: false});
            }

        } else if (bulkItem === true) {  //Bulk Item
                //set Primary to 'Bulk'
            rec.setValue({fieldId: 'custrecord_br_itdv_primary_class', value: 'Bulk'});
                //set Primary Fields
            rec.setValue({fieldId: 'custrecord_br_itdv_contrib_margin_costs', value: bulkContributionMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_contribution_margin', value: bulkContributionMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_contrib_margin_pct', value: bulkContributionMarginPct.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_total_unit_costs', value: bulkGrossMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_gross_margin', value: bulkGrossMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_gross_margin_pct', value: bulkGrossMarginPct.toFixed(2)});
            ...
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_gross_margin_pct', value: fbmGrossMarginPct.toFixed(2)});
                ...
                //set Primary to 'FBM'
            rec.setValue({fieldId: 'custrecord_br_itdv_primary_class', value: 'FBM'});
                //set Primary Fields
            rec.setValue({fieldId: 'custrecord_br_itdv_contrib_margin_costs', value: fbmContributionMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_contribution_margin', value: fbmContributionMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_contrib_margin_pct', value: fbmContributionMarginPct.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_total_unit_costs', value: fbmGrossMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_gross_margin', value: fbmGrossMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_gross_margin_pct', value: fbmGrossMarginPct.toFixed(2)});

                //Set Alternate to 'FBA'
            rec.setValue({fieldId: 'custrecord_br_itdv_alternate', value: 'FBA'});
                //Set Alternate Fields
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_margin_costs', value: fbaContributionMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_margin', value: fbaContributionMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_margin_pct', value: fbaContributionMarginPct.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_unit_costs', value: fbaGrossMarginCosts.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_gross_margin', value: fbaGrossMarginAmt.toFixed(2)});
            rec.setValue({fieldId: 'custrecord_br_itdv_alt_gross_margin_pct', value: fbaGrossMarginPct.toFixed(2)});
